    ...
    namespace Lab9
    {
        ...
        new { ID=175, Name="Wicked Witch", Job="Queen" }
            ...
            // Query 0
            IEnumerable<int> oddsQuery =
                from num in odds
                where num % 3 == 0
                select num;
            Console.WriteLine("Numbers that are divisible by 3 in odds are: ");
            foreach (int x in oddsQuery)
            {
            ...
            //query1
            IEnumerable<int> queryOne =
              from a in odds
              from b in evens
              where b > (2 * a)
              select b;
            Console.WriteLine("Query1 ");
            foreach (int x in queryOne)
            {
            ...
            //query2
            IEnumerable<Object> queryTwo =
           from a in data

           where (a.ID > 100 && (a.ID < 500)) && a.Name.Contains('y') || a.Job.EndsWith("e") || a.Job.Contains('e') || a.Job == "Student" || a.Job == "Programmer"
           select a;
            Console.WriteLine("Query2  ");
            foreach (Object obj in queryTwo)
            {
            ...
            IEnumerable<Product> queryThree =
            ...
            foreach (Product prod in queryThree)
            {
                
               Console.WriteLine("ProductName: "+prod.ProductName +" Units in stock: " +prod.UnitsInStock);
            }

            //query 4 
            //List to use when using the join clause
            List<Product> productListX = new List<Product>
                        {
                ...
                List < Product> listZ = GetProductList();
                IEnumerable<Product> queryFour =
                                            from pro in listZ
                                            where pro.ProductID < 5
                                            //assign pro to a new variable
                                            let product=pro
               
                                            join productZ in productListX on product.ProductID equals productZ.ProductID
                                           //descending order
                                            orderby product.UnitsInStock descending
           
                                            select product;

                foreach (Product prod in queryFour)
                {
    ...
    new Product{
                        ProductID = 9, ProductName = "Mishi Kobe Niku", Category =
    ...
    new Product{
                        ProductID = 11, ProductName = "Queso Cabrales", Category =
    ...
    new Product{
                        ProductID = 24, ProductName = "Guaraná Fantástica", 
